...
<script src="<c:url value="/resources/js/wuliu/wuliuSj.js"/> "></script>
...
<div class="tab_div">
    <span class="tab_nav">
        ...
        <a  href="/SINOFAITH/wuliuSj" class="addactive">物流收件人信息</a>
    </span>
    <ul >
                ...
                <div class="cantent_block ">
                            ...
                            <div >
                                ...
                                <table class="table  table-hover table_style table_list1 " style="border-left: 1px solid #ccc; border-right: 1px solid #ccc!important;">
                                    ...
                                    <tr align="center">
                                        <td width="6%">序号</td>
                                        <td width="6%">收件人</td>
                                        <td width="6%"><a href="/SINOFAITH/wuliuSj/seach?pageNo=1&orderby=sj_phone">收件电话</a></td>
                                        <td width="12%">收件地址</td>
                                        <td width="6%"><a href="/SINOFAITH/wuliuSj/seach?pageNo=1&orderby=num">收件次数</a></td>
                                    </tr>
                                    <c:forEach items="${detailinfo}" var="item" varStatus="st">
                                        <tr class="${st.index%2==1 ? '':'odd' }">
                                            ...
                                            <td align="center" title="${item.sj_address}"><div style="width:250px;white-space: nowrap;text-overflow:ellipsis; overflow:hidden;">${item.sj_address}</div></td>
                                            <td align="center">
                                                <button class="btna" data-toggle="modal" data-target="#myModal" onclick="getZzDetails(this)">${item.num}</button>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                    <c:choose>
                            ...
                            <c:choose>
                                <c:when test="${detailinfo!=null && detailinfo.size()!=0}">
                                    <div  class="page_nmber">
                                        ...
                                        <a href="/SINOFAITH/wuliuSj/seach?pageNo=${page.topPageNo }"><input type="button" name="fristPage" value="首页" /></a>
                                        <c:choose>
                                            <c:when test="${page.pageNo!=1}">
                                                <a href="/SINOFAITH/wuliuSj/seach?pageNo=${page.previousPageNo }"><input type="button" name="previousPage" value="上一页" /></a>
                                            </c:when>
                                            <c:otherwise>
                                        ...
                                        <c:choose>
                                            <c:when test="${page.pageNo != page.totalPages}">
                                                <a href="/SINOFAITH/wuliuSj/seach?pageNo=${page.nextPageNo }"><input type="button" name="nextPage" value="下一页" /></a>
                                            </c:when>
                                            <c:otherwise>
                                                <input type="button" disabled="disabled" name="nextPage" value="下一页" />
                                            </c:otherwise>
                                        </c:choose>
                                        <a href="/SINOFAITH/wuliuSj/seach?pageNo=${page.bottomPageNo }"><input type="button" name="lastPage" value="尾页" /></a>
                                        <input type="number" id="num" max="${page.totalPages}" style="width: 9%" min="1">
                                        <input type="button" value="跳转" onclick="relationSkip('wuliuSj')">
                                    </div>
                    ...
                    <div class="sidebar_right pos_re">
                        ...
                        <div class=" ">
                            ...
                            <div>
                                <form action="/SINOFAITH/wuliuSj/SeachCode" method="post">
                                    <div class="form-group_search  fl_l width100" >
                                        <span style="margin-left: 10px;color: #444;padding-bottom: 10px;">查询方式</span>
                                        <select name="seachCondition" class="width100" STYLE="margin-bottom: 20px;">
                                            <option value="addressee"<c:if test="${wuliuSjSeachCondition=='addressee'}">selected="selected"</c:if>>收件人</option>
                                            <option value="sj_phone"<c:if test="${wuliuSjSeachCondition=='sj_phone'}">selected="selected"</c:if>>收件电话</option>
                                        </select>
                                        <%--<input  style="margin-left: 10px;" type="checkbox" name="usable" value="1" <c:if test="${usable eq '1'}">checked="checked"</c:if>>上次条件有效--%>
                                        <textarea  class="form-control02 seachCode fl_l width100" id="seachCode" placeholder="请输入要查询内容" name="seachCode" >${wuliuSjSeachCode}</textarea>
                                    </div>
                       ...
                       <span class="fl_l width100 " style="padding-bottom: 10px;margin-top: 10px;">
                           ...
                           <%--<c:if test="${!fn:contains(aj.aj, ',')}">
                               ...
                               <button  type="button"  class="sideBar_r_button" <c:if test="${aj!=null && detailinfo.size()!=0}">onclick="location.href='/SINOFAITH/wuliuSj/download'"</c:if>>数据导出</button>
                       </span>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="modal-dialog" style="top: 0%; min-width: 90%;left: 5%;right: 5%;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">收收详情<span id="title"></span></h4>
            </div>
            <div class="modal-body">
                <table class="table  table-hover table_style table_list1 " style="border-left: 1px solid #ccc; border-right: 1px solid #ccc!important;">
                    ...
                    <tr align="center">
                        ...
                        <td width="8%">运单号</td>
                        <td width="10%">
                            <button onclick="orderByFilter('ship_time')">收件时间</button>
                        </td>
                        <td width="8%">寄件人</td>
                        <td width="8%">寄件电话</td>
                        <td width="15%">寄件地址</td>

...
(function ($) {
	...
	function loadData(data, keyField) {
		...
		for (var row = startRow - 1; row < ((base.currentPageIdx + 1) * base.pageSize) && row < data.length-1; ++row)
		{
			// start a new row
		...
		// update the footer
		renderFooter({ start: startRow, end: curRow - 1, total: data.length-1, size: data[data.length-1]['size'] });
	};

	// sets all the options
	jQuery.fn.raytable = function (options) {
		base.maxPageButtons = 10;
	...
	function renderFooter(params) {
		...
		// update the summary
		base.parentElem.find('#raytable-footer-summary').text(params.start + ' - ' + params.end + ' 共 ' + params.size + ' 条记录');
		...
		var summary = '<span id="raytable-footer-summary" style="float:right;">' + params.start + ' - ' + params.end + ' 共 ' + params.size + ' 条记录</span>';

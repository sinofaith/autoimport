...
public class PsHierarchyDao extends BaseDao<PsHierarchyEntity>{
    ...
    public int getRowPsHierarchy(String seach, long id) {
        ...
        sql.append(" select p1.id,p1.psid,p1.sponsorid,p1.accountholder psAccountholder,p1.sfzhm,p1.mobile,p1.address,p1.accountnumber,h.tier,h.path,h2.sum directDrive,c.containsTier,c.directrefernum from (");
        sql.append(" select p.id,p.psid,p.sponsorid,p.accountholder,p.sfzhm,p.mobile,p.address,p.accountnumber from(select * from (select t.*,row_number() over(partition by t.psid,t.sponsorid order by t.id)");

...
public class BankZzxxDao extends BaseDao<BankZzxxEntity>{
    public int getAllRowCount(String seachCode){
        String sql = "select to_char(count(1)) num from bank_zzxx c " +
                "left join bank_zcxx s on c.yhkkh = s.yhkkh " +
                "left join bank_zcxx d on c.dskh = d.yhkkh where 1=1 " + seachCode;
    ...
    public List getDoPage(String seach,int offset,int length){
        ...
        sql.append("FROM  bank_zzxx c left join bank_zcxx s on c.yhkkh=s.yhkkh ");
        sql.append("left join bank_zcxx d on c.dszh=d.yhkkh  where 1=1 "+seach+") a ");

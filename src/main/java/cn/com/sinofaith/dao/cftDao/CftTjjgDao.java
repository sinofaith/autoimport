...
//    }
    ...
    public void save(List<CftTjjgEntity> tjjgs){
        ...
        String sql = "insert into cft_tjjg(jyzh,jylx,jyzcs,jzzcs,jzzje,czzcs,czzje,inserttime,aj_id,minsj,maxsj) " +
                "values(?,?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            ...
            for (int i = 0; i < tjjgs.size(); i++) {
                ...
                st.setString(10,tjjg.getMinsj());
                st.setString(11,tjjg.getMaxsj());
                st.addBatch();
                if((i+1)%50000 == 0){
                    st.executeBatch();
                    con.commit();
